...
DELIMITER ;;
/*!50003 CREATE*/ /*!50017 DEFINER=`root`@`localhost`*/ /*!50003 TRIGGER `trg_loan_insert_audit` AFTER INSERT ON `loan_application` FOR EACH ROW BEGIN
    INSERT INTO Audit_Log
        (application_id, action, performed_by, new_value)
    VALUES
        (NEW.application_id, 'APPLICATION_CREATED', 'SYSTEM', 'SUBMITTED');
END */;;
DELIMITER ;
...
/*!50003 SET sql_mode              = 'ONLY_FULL_GROUP_BY,STRICT_TRANS_TABLES,NO_ZERO_IN_DATE,NO_ZERO_DATE,ERROR_FOR_DIVISION_BY_ZERO,NO_ENGINE_SUBSTITUTION' */ ;
DELIMITER ;;
/*!50003 CREATE*/ /*!50017 DEFINER=`root`@`localhost`*/ /*!50003 TRIGGER `trg_block_critical_risk_approval` BEFORE UPDATE ON `loan_application` FOR EACH ROW BEGIN
    DECLARE v_risk_cat VARCHAR(20);
    IF NEW.status = 'APPROVED' THEN
        SELECT risk_category INTO v_risk_cat
        FROM Risk_Assessment
        WHERE application_id = NEW.application_id
        ORDER BY created_at DESC
        LIMIT 1;
        IF v_risk_cat IN ('CRITICAL', 'VERY_HIGH') THEN
            SIGNAL SQLSTATE '45000'
            SET MESSAGE_TEXT = 'Cannot approve: risk is CRITICAL or VERY_HIGH';
        END IF;
    END IF;
END */;;
DELIMITER ;
...
/*!50003 SET sql_mode              = 'ONLY_FULL_GROUP_BY,STRICT_TRANS_TABLES,NO_ZERO_IN_DATE,NO_ZERO_DATE,ERROR_FOR_DIVISION_BY_ZERO,NO_ENGINE_SUBSTITUTION' */ ;
DELIMITER ;;
/*!50003 CREATE*/ /*!50017 DEFINER=`root`@`localhost`*/ /*!50003 TRIGGER `trg_set_risk_category` BEFORE INSERT ON `risk_assessment` FOR EACH ROW BEGIN
    IF NEW.risk_score >= 85     THEN SET NEW.risk_category = 'CRITICAL';
    ELSEIF NEW.risk_score >= 65 THEN SET NEW.risk_category = 'HIGH';
    ELSEIF NEW.risk_score >= 45 THEN SET NEW.risk_category = 'MEDIUM';
    ELSE                             SET NEW.risk_category = 'LOW';
    END IF;
END */;;
DELIMITER ;
/*!50003 SET sql_mode              = @saved_sql_mode */ ;
...
-- Dump completed on 2026-04-28 20:12:34
